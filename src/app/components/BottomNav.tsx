...
export function BottomNav({ active, onChange }: BottomNavProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#6b1a1a]/10 z-40 safe-bottom">
      <div className="flex">
        {tabs.map(tab => {
          ...
          return (
            <button
              ...
              className="flex-1 flex flex-col items-center justify-center pt-2 pb-3 gap-0.5 transition-all"
            >
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${isActive ? "bg-[#6b1a1a]" : "bg-transparent"}`}>
                <tab.icon size={17} color={isActive ? "#ffffff" : "#8a5a5a"} />
              </div>
              <span
                className="text-[9px] font-medium transition-colors"
                style={{ color: isActive ? "#6b1a1a" : "#8a5a5a" }}
              >
                {tab.label}
              </span>
            </button>
